---
role: Data Science Developer Intern
employer: SAP
location: Singapore
start_date: 2024-05-06
end_date: 2024-07-26
layout: default
summary: I was a Data Science Developer Intern at SAP Singapore working in the Personalized Recommendations team. My work involved performing several benchmarking experiments pertaining to training and finetuning foundational models for PR.
last_updated: 2025-12-26
---

# Data Science Developer Intern
{% if page.start_date %} {{ page.start_date | date: "%b %-d, %Y" }}{% endif %}
{% if page.end_date %} — {{ page.end_date | date: "%b %-d, %Y" }}
{% else %} — Present {% endif %}

<p style="text-align: right;">
  <small>Last Updated: {{ page.last_updated | date: "%b %-d, %Y" }}</small>
</p>

I was a Data Science Developer Intern at [SAP](https://www.sap.com/sea/index.html) Singapore working in the Personalized Recommendations team. My work involved performing several benchmarking experiments pertaining to training and finetuning foundational models for PR.
 
My experience as an intern allowed me to gain a deeper understanding of the AI, and I learnt so much more about Generative AI, LLMs, neural networks, deep learning, and recommendation models.

I was able to enhance several key skills such as PyTorch, Python programming, LangChain, prompt engineering, data analysis, data science, and technical documentation. 

I also had the opportunity to participate in the GenAI Hackathon organized by SAP Labs, which allowed me to put my newly learnt skills to the test. All ideas were voted upon internally and our idea was ranked in the top 13% of ideas submitted globally.

<hr style="border: 1px solid #ccc; margin: 3rem 0;">

## Photo Highlights
<div style="display: flex; flex-wrap: wrap; gap: 1rem; justify-content: center;">
  <figure style="width: 48%; text-align: center;">
    <img src="{{ '/assets/images/sap/interns_2.jpg' | relative_url }}" alt="Me with fellow interns" style="width: 100%; height: auto;">
    <figcaption>Working on the internal GenAI Hackathon</figcaption>
  </figure>
  <figure style="width: 48%; text-align: center;">
    <img src="{{ '/assets/images/sap/interns.jpg' | relative_url }}" alt="Another image with interns" style="width: 100%; height: auto;">
    <figcaption>Me with my fellow interns</figcaption>
  </figure>
  <figure style="width: 48%; text-align: center;">
    <img src="{{ '/assets/images/sap/sap_team.jpg' | relative_url }}" alt="SAP Team" style="width: 100%; height: auto;">
    <figcaption>Team lunch</figcaption>
  </figure>
  <figure style="width: 48%; text-align: center;">
    <img src="{{ '/assets/images/sap/sap_social.jpg' | relative_url }}" alt="Team Social" style="width: 100%; height: auto;">
    <figcaption>Team Social!</figcaption>
  </figure>
  <figure style="width: 48%; text-align: center;">
    <img src="{{ '/assets/images/sap/ben_and_me.jpg' | relative_url }}" alt="Me with my internship mentor, Ben" style="width: 100%; height: auto;">
    <figcaption>Me with my internship mentor, Ben, on my last day.</figcaption>
  </figure>
</div>